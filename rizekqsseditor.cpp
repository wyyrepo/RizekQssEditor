	...
	: QMainWindow(parent, flags)
{
	...
	connect(plainTextEdit, SIGNAL(textChanged()), this, SLOT(setPlainTextEditModified())); //óńòàíàâëèâàćò ńâîéńòâî íà äà ćńëè òćêńò èçìćíÿëè
...
void RizekQssEditor::rewriteRecentProjects()
{
	...
	for (int a=0;a<6;++a)
	{
		if (settings.value("lastProjects/" + QString::number(a)).toString().isEmpty()) //Èùćò ïóńòóț ńòđîêó
		{
			bool isNoCopy = true;
			for (int b=0;b<a;++b)
			{
					if (addressOfCurrenFile==settings.value("lastProjects/" + QString::number(b)).toString()) //Èùćò ńîâïàäćíèÿ
					{
						isNoCopy = false;
					}
			}
			if(isNoCopy && isListNotComplete) //Ćńëè íćò êîïèé òî çàïèńàòü
				{
	...
	if (isListNotComplete==false)
	{
		...
		if (isNoCopy==true)//Ćńëè íćò êîïèé
		{
			for(int a=5;a>0;--a)
			{
...
{
	QObject *o = sender(); // Óçíàćì êàêîé îáúćêò îòïđàâèë ńèăíàë (âûçâàë ńëîò)
	QAction * act = qobject_cast<QAction *>(o); // ïîïđîáóćì ïđèâćńòè òèï ęòîăî îáúćêòà ê îæèäàćìîìó (QAction)
	if (act)
		if(act->text()!=addressOfCurrenFile)
		{
			...
			for(int a=1;a<6;++a)
				if (settings.value("lastProjects/" + QString::number(a)).toString()==act->text()) 
					for(int b=a;b>0;--b) //Ńáđàńûâàćì çàïèńè ïćđćä íóæíîé íà îäíó íàçàä
			...
			openFileFromString(act->text()); // ćńëè ïđèâćäćíèć òèïà óäàëîńü, òî âûçîâèì íóæíûé ìćòîä îæèäàćìîăî îáúćêòà.
		}
}
